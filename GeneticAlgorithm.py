...
class GeneticAlgorithm:
    ...
    def __init__(self, matrix=[], init_population_size=50, mutation_chance=10, max_population_size=100, rate_cross = 0.5):
        ...
        for i in range(self.num_of_regions):
            for j in range(i + 1, self.num_of_regions):
                if adjacency_matrix[i][j] == 1 and chromosome[i] == chromosome[j]:
                    fitness_score -= 10
        if fitness_score == -10  and maxloop>=0 and random.randint(0, 1) == 1:
        ...
        while True:
            ...
            for i in range(int(sz*self.rate_cross)):
                for j in range(i + 1, sz):
                    cross_rand = np.random.randint(0, 5)
                    if cross_rand == 0:
            ...
            if fitness_scores[-1] == self.max_possible_score or a==50:
